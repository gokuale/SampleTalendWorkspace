...
public class TFixedFlowInputSparkconfigJava
{
  protected static String nl;
  public static synchronized TFixedFlowInputSparkconfigJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TFixedFlowInputSparkconfigJava result = new TFixedFlowInputSparkconfigJava();
    nl = null;
    return result;
  }
  ...
  protected final String TEXT_60 = NL + NL + "org.apache.spark.api.java.JavaPairRDD<NullWritable, ";
  protected final String TEXT_61 = "> rdd_";
  protected final String TEXT_62 = NL + "        = ctx.parallelizePairs(new Data_";
  protected final String TEXT_63 = "().getPairList(), ";
...
String nbRows = tFixedFlowInputUtil.nbRows;
int rowCount = 2;

boolean definePartitionNumber = "true".equals(ElementParameterParser.getValue(node, "__DEFINE_PARTITION_NUMBER__"));
if(definePartitionNumber) {
    nbRows = ElementParameterParser.getValue(node, "__PARTITION_NUMBER__");
    try {
        rowCount = Integer.parseInt(nbRows);
    } catch (java.lang.NumberFormatException e) {
        // Use the default value if the parsing fails.
    }
} else {
    try {
        rowCount = Integer.parseInt(nbRows);
        rowCount = (rowCount <= 1) ? 2 : rowCount;
    } catch (java.lang.NumberFormatException e) {
        // Use the default value if the parsing fails.
    }
}
